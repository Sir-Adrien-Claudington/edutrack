...
export async function confirmMfa(req: AuthRequest, res: Response) {
  const { code } = req.body
  if (!code || typeof code !== 'string') { res.status(400).json({ error: 'TOTP code required' }); return }

  const user = await prisma.user.findUnique({ where: { id: req.user!.id } })
  if (!user || !user.mfaSecret) { res.status(400).json({ error: 'MFA setup not started' }); return }
...
export async function verifyMfa(req: AuthRequest, res: Response) {
  ...
  if (!mfaToken || !code) { res.status(400).json({ error: 'mfaToken and code required' }); return }

  let payload: { id: string; scope?: string }
  try {
    payload = jwt.verify(mfaToken, process.env.JWT_SECRET!) as { id: string; scope?: string }
  } catch {
    res.status(401).json({ error: 'Invalid or expired MFA token' }); return
  }

  if (payload.scope !== 'mfa') { res.status(401).json({ error: 'Invalid token scope' }); return }

  const user = await prisma.user.findUnique({ where: { id: payload.id } })
  if (!user || !user.mfaEnabled || !user.mfaSecret) {
    res.status(401).json({ error: 'MFA not configured for this account' }); return
  }
