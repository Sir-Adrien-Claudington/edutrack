...
export async function updateExternalAssignment(req: AuthRequest, res: Response) {
  const { id } = req.params
  const a = await prisma.externalAssignment.findUnique({
    where: { id },
    include: { classroom: true },
  })
  if (!a || a.classroom.teacherId !== req.user!.id) {
    res.status(403).json({ error: 'Forbidden' })
    return
  }
...
export async function curveExternalAssignment(req: AuthRequest, res: Response) {
  ...
  const a = await prisma.externalAssignment.findUnique({
    where: { id },
    include: { classroom: true, externalGrades: true },
  })
  if (!a || a.classroom.teacherId !== req.user!.id) {
    res.status(403).json({ error: 'Forbidden' })
    return
  ...
  await Promise.all(
    grades.map((g) => {
      ...
      if (curveType === 'flat') newScore = Math.min(a.totalMarks, base + Number(value))
        ...
        newScore = Math.round(Math.sqrt(base / a.totalMarks) * a.totalMarks * 10) / 10
      else if (curveType === 'scale_max')
        newScore = maxScore > 0 ? Math.min(a.totalMarks, (base / maxScore) * a.totalMarks) : base
      return prisma.externalGrade.update({
        where: { id: g.id },
        data: { score: Math.round(newScore * 10) / 10 },
      })
    })
  )

  const updated = await prisma.externalAssignment.findUnique({
    where: { id },
    include: { externalGrades: true },
  })
  res.json(updated)
