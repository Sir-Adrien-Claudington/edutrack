...
export async function exportClassroomCSV(req: AuthRequest, res: Response) {
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="${safeName(classroom.name)}-report.csv"`
  )
  res.send(csv)
}
...
export async function exportStudentReport(req: AuthRequest, res: Response) {
  ...
  const submissions = await prisma.submission.findMany({
    ...
    include: {
      assignment: {
        select: { title: true, totalPoints: true, classroom: { select: { name: true } } },
      },
      feedback: true,
    },
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="${safeName(student.name)}-report.csv"`
  )
  res.send(csv)
}
...
export async function exportStudentPDF(req: AuthRequest, res: Response) {
  ...
  const boundaries = await (async () => {
    const b = await prisma.gradeBoundary.findMany({
      where: { teacherId: req.user!.id },
  ...
  const externalAssignments = sections.assignments
    ? await prisma.externalAssignment.findMany({
        where: { classroomId },
  ...
  const comment = sections.comments
    ? await prisma.studentComment.findUnique({
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="${safeFileName(student.name)}_Report.pdf"`
  )
  doc.pipe(res)
  ...
  if (units.length > 0) {
    doc
      ...
      .fillColor(indigo)
      .font('Helvetica-Bold')
      .text('Units included in this report:', { align: 'center' })
  ...
  if (teacherLevels.length > 0) {
    ...
    doc
      ...
      .fillColor(indigo)
      .font('Helvetica-Bold')
      .text('Understanding Level Key', { align: 'center' })
    ...
    teacherLevels.forEach((l) => {
      ...
      doc
        .fontSize(9)
        .fillColor('#374151')
        .text(`   ${l.label}${suffix}`, 260, legendY + 1)
      doc.moveDown(0.35)
    })
  }
  ...
  for (const unit of units) {
    ...
    doc
      ...
      .fillColor(gray)
      .font('Helvetica')
      .text(activeTerm?.name ?? classroom.name)
    doc.moveDown(0.5)
    hline(doc.y)
    doc.moveDown(0.5)
    ...
    if (sections.lessons && unit.lessons.length > 0) {
      ...
      for (const lesson of unit.lessons) {
        ...
        doc
          .fontSize(9)
          .fillColor('#111827')
          .text(lesson.title, 54, rowY, { width: lessonColW[0] - 4 })
      ...
      if (summative) {
        doc
          .fontSize(9)
          .fillColor(gray)
          .text('Summative Unit Result: ', 54, summResultY, { continued: true })
        doc.rect(54 + 140, summResultY - 1, 120, 14).fill(summative.colour)
        doc
          .fontSize(9)
          .fillColor(contrastText(summative.colour))
          .font('Helvetica-Bold')
        ...
        doc
          .fontSize(9)
          .fillColor(gray)
          .text('Summative Unit Result: No attended lessons recorded', 54, summResultY)
      }
      doc.moveDown(0.8)
    }
    ...
    if (sections.assessment && unit.unitAssessments.length > 0) {
      ...
      const pct =
        assessment.score !== null ? (assessment.score / assessment.totalMarks) * 100 : null
      ...
      doc
        ...
        .fillColor('#111827')
        .text(
          `Score: ${assessment.score ?? '—'} / ${assessment.totalMarks}   ·   ${pct !== null ? pct.toFixed(1) + '%' : '—'}${g ? `   (${g.label})` : ''}`
        )
      ...
      if (g) {
        ...
        doc
          ...
          .fontSize(9)
          .font('Helvetica-Bold')
          .text(g.label, 50, badgeY + 5, { width: 90, align: 'center' })
  ...
  if (sections.assignments && externalAssignments.length > 0) {
    ...
    headers.forEach((h, i) => {
      doc
        ...
        .fillColor(gray)
        .font('Helvetica-Bold')
        .text(h, x + 2, hdrY + 4, { width: colW[i] })
      x += colW[i]
    })
    ...
    if (hasNonDefaultWeight) {
      ...
      doc
        .fontSize(8)
        .fillColor(gray)
        .text('* Weighted average accounts for individual assignment weights')
    }
  }
