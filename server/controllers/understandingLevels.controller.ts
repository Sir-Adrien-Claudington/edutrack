...
export async function createLevel(req: AuthRequest, res: Response) {
  ...
  if (!label || !colour) {
    res.status(400).json({ error: 'label and colour required' })
    return
  }
...
export async function updateLevel(req: AuthRequest, res: Response) {
  ...
  if (!level || level.teacherId !== req.user!.id) {
...
export async function deleteLevel(req: AuthRequest, res: Response) {
  ...
  const removedFromLessons = await prisma.lessonUnderstanding.count({
    where: { understandingLevelId: id },
  })
