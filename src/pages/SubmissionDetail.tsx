    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <nav className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center gap-4">
        ...
        {role === 'TEACHER' && hasManualQuestions && submission.status !== 'GRADED' && (
          <span className="ml-auto text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-2 py-1 rounded-full font-medium">
            Needs grading
          </span>
        )}
      </nav>
      ...
      <main className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 flex items-center justify-between">
          ...
          {submission.totalScore !== null && (
            <div
              className={`text-3xl font-bold ${submission.totalScore >= 70 ? 'text-teal-600' : 'text-orange-500'}`}
            >
        ...
        {submission.feedback?.aiSuggestion && (
          <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4">
            <span className="text-xs font-semibold text-indigo-700 uppercase tracking-wide bg-indigo-100 px-2 py-0.5 rounded">
              AI Feedback
            </span>
        ...
        {role === 'STUDENT' && submission.feedback?.teacherNote && (
          <div className="bg-teal-50 border border-teal-200 rounded-xl p-4">
            ...
            <p className="text-sm text-teal-900 leading-relaxed mt-2">
              {submission.feedback.teacherNote}
            </p>
          </div>
        )}
            ...
            >
              ...
              <div className="flex items-center justify-between mt-3 text-xs">
                {a.isCorrect !== null ? (
                  <span
                    className={
                      a.isCorrect ? 'text-teal-600 font-medium' : 'text-orange-600 font-medium'
                    }
                  >
                  ...
                  <span className="text-gray-400 dark:text-gray-500 italic">
                    {role === 'TEACHER' ? 'Set points →' : 'Manually graded'}
                  </span>
                )}
        ...
        {role === 'TEACHER' && (
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 space-y-4">
            ...
            <div>
              <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">
                ...
                <span className="text-gray-400 font-normal">
                  (optional — shown on their submission view)
                </span>
              </label>
              <textarea
                value={teacherNote}
            ...
            <div className="flex items-center justify-between">
              {saveMsg ? (
                <span
                  className={`text-sm font-medium ${saveMsg.startsWith('Saved') ? 'text-teal-600' : 'text-red-500'}`}
                >
                  {saveMsg}
                </span>
              ) : (
