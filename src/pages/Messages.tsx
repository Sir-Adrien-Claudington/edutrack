...
export default function Messages() {
  ...
  const [classrooms, setClassrooms] = useState<
    { id: string; name: string; enrollments?: { student: { id: string; name: string } }[] }[]
  >([])
  ...
  async function send(e: React.FormEvent) {
    ...
    try {
      ...
      if (user?.role === 'TEACHER' && selectedClassroom && !recipientId)
  ...
  const students = classrooms.flatMap((c) =>
    (c.enrollments ?? []).map((e) => ({ ...e.student, classroomName: c.name }))
  )
          ...
          <form onSubmit={send} className="space-y-3">
            ...
            {user?.role === 'STUDENT' && students.length === 0 && (
              <p className="text-sm text-gray-400">
                You need to enroll in a class before messaging your teacher.
              </p>
            )}
            <textarea
              value={body}
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {messages.map((m) => (
              <div
                key={m.id}
                className={`bg-white dark:bg-gray-800 border rounded-xl px-5 py-4 ${!m.readAt && m.sender.id !== user?.id ? 'border-indigo-300 bg-indigo-50 dark:border-indigo-700 dark:bg-indigo-900/20' : 'border-gray-200 dark:border-gray-700'}`}
              >
