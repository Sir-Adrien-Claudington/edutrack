...
export default function Content() {
  ...
  async function handleOverrideGrade() {
    ...
    setSaving(true)
    try {
      await api.put(`/admin/content/submissions/${gradeModal.id}/grade`, {
        totalScore: Number(gradeValue),
        curveNote: gradeNote || undefined,
      })
      setSubmissions((s) => ({
        ...s,
        ...Object.fromEntries(
          Object.entries(s).map(([aid, subs]) => [
            aid,
            subs.map((sub) =>
              sub.id === gradeModal.id
                ? { ...sub, totalScore: Number(gradeValue), status: 'GRADED' }
                : sub
            ),
          ])
        ),
  ...
  async function handleDeleteSubmission(assignmentId: string, submissionId: string) {
    ...
    setSubmissions((s) => ({
      ...s,
      [assignmentId]: s[assignmentId].filter((sub) => sub.id !== submissionId),
    }))
  }
    ...
    setSubmissions((s) => ({
      ...
      ...Object.fromEntries(
        Object.entries(s).map(([aid, subs]) => [
          aid,
          subs.map((item) =>
            item.id === sub.id ? { ...item, plagiarismFlag: false, plagiarismReport: null } : item
          ),
        ])
      ),
    }))
      ...
      <div className="p-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            View all assignments and override grades
          </p>
        </div>
                ...
                {assignments.map((a) => (
                  <>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">
                        {a.classroom.teacher.name}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-medium ${TYPE_COLOURS[a.type] ?? ''}`}
                        >
                          {a.type}
                        </span>
                      </td>
                                ...
                                >
                                  ...
                                  <div className="flex gap-1">
                                    <button
                                      onClick={() => {
                                        setGradeModal(sub)
                                        setGradeValue(String(sub.totalScore ?? ''))
                                        setGradeNote('')
                                      }}
                                    ...
                                    <button
                                      onClick={() => handleDeleteSubmission(a.id, sub.id)}
                                      className="px-2 py-1 text-xs text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded"
                                    >
                                      Delete
                                    </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              This will delete the assignment and all its submissions permanently.
            </p>
            <div className="flex justify-end gap-2 mt-4">
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
              {plagiarismModal.student.name}
            </p>
            <p className="text-sm text-gray-700 dark:text-gray-300 mb-4">
              {plagiarismModal.plagiarismReport ?? 'No details available'}
            </p>
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={() => handleDismissPlagiarism(plagiarismModal)}
