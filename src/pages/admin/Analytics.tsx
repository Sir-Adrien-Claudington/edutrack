...
export default function Analytics() {
  ...
  useEffect(() => {
    Promise.all([api.get('/admin/analytics/stats'), api.get('/admin/analytics/usage')])
      .then(([s, u]) => {
        setStats(s.data)
        setUsage(u.data)
      })
  ...
  const gradingRate =
    stats && stats.submissionCount > 0
      ? Math.round((stats.gradedCount / stats.submissionCount) * 100)
      : 0

  const barData =
    usage?.classroomActivity.slice(0, 8).map((c) => ({
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
          {[
            ...
            },
            {
              ...
              colour: 'text-gray-900 dark:text-white',
            },
            {
              label: 'Submissions',
            ...
            <div className="flex items-center justify-center h-[220px]">
              <div className="text-center">
                ...
                <div className="text-xs text-gray-400 mt-1">
                  {stats?.gradedCount} / {stats?.submissionCount}
                </div>
              ...
              {(stats?.recentUsers ?? []).map((u) => (
                <div key={u.id} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${u.role === 'TEACHER' ? 'bg-blue-100 text-blue-700' : u.role === 'ADMIN' ? 'bg-purple-100 text-purple-700' : 'bg-teal-100 text-teal-700'}`}
                    >
                      {u.role}
                    </span>
                ...
                <div key={t.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400">
                      {t.classrooms} class · {t.totalStudents} students · last login{' '}
                      {timeAgo(t.lastLogin)}
                    </p>
                  </div>
                  <span className="text-xs font-medium text-indigo-600 dark:text-indigo-400">
                    {t.totalAssignments} assign.
                  </span>
                </div>
              ))}
              {(usage?.teacherStats ?? []).length === 0 && (
