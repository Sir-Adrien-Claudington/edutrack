...
export default function Classrooms() {
  ...
  async function removeStudent(classroomId: string, studentId: string) {
    ...
    setStudents((s) => ({
      ...s,
      [classroomId]: s[classroomId].filter((st) => st.id !== studentId),
    }))
  }
      ...
      <div className="p-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Manage all classrooms on the platform
          </p>
        </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {classrooms.map((c) => (
                  <>
                    <tr
                      key={c.id}
                      className={`hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors ${c.archived ? 'opacity-60' : ''}`}
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-medium ${c.archived ? 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400' : 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400'}`}
                        >
                                ...
                                >
                                  <div>
                                    ...
                                    <p className="text-xs text-gray-400">
                                      {st.email ?? '—'} · Last login: {timeAgo(st.lastLoginAt)}
                                    </p>
                                  </div>
                                  <button
                                    onClick={() => removeStudent(c.id, st.id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              This will permanently delete the classroom and all its assignments, submissions, and
              grades. This cannot be undone.
            </p>
