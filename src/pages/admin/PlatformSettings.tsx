...
const SETTING_LABELS: Record<string, { label: string; description: string }> = {
  ...
  studentRegistrationEnabled: {
    ...
    description: 'Allow new students to self-register',
  },
...
export default function PlatformSettings() {
  ...
  useEffect(() => {
    Promise.all([api.get('/admin/platform/settings'), api.get('/admin/platform/announcements')])
      .then(([s, a]) => {
        setSettings(s.data)
      ...
      <div className="p-8">
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
            {settings.map((s) => {
              ...
              return (
                <div key={s.key} className="flex items-center justify-between px-6 py-4">
                  ...
                  >
                    <span
                      className={`inline-block h-4 w-4 rounded-full bg-white shadow transition-transform ${s.value === 'true' ? 'translate-x-6' : 'translate-x-1'}`}
                    />
                  </button>
                </div>
              )
            ...
            {saved && (
              <div className="px-6 py-3 text-xs text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20">
                Saved
              </div>
            )}
          </div>
                  ...
                  {announcements.map((ann) => (
                    <div key={ann.id} className="flex items-start justify-between px-5 py-4">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-medium ${ann.active ? 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400' : 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400'}`}
                        >
