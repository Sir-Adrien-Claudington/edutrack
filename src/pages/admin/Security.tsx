      ...
      <div className="p-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Admin action log and user session management
          </p>
        </div>
                    ...
                    {logs.map((log) => (
                      <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                        <td className="px-4 py-3">
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${ACTION_COLOURS[log.action] ?? 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'}`}
                          >
                            {log.action}
                          </span>
                        </td>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {history.user.email} · Joined{' '}
                      {new Date(history.user.createdAt).toLocaleDateString()}
                    </p>
                    <p className="text-xs text-gray-400">
                      Last login:{' '}
                      {history.user.lastLoginAt
                        ? new Date(history.user.lastLoginAt).toLocaleString()
                        : 'Never'}
                    </p>
                  </div>
                  <button
                  ...
                  <div className="space-y-2">
                    {history.auditEntries.map((e) => (
                      <div key={e.id} className="flex items-center gap-3 text-sm">
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${ACTION_COLOURS[e.action] ?? 'bg-gray-100 text-gray-600'}`}
                        >
                          {e.action}
                        </span>
