...
export default function GradeTracker() {
  ...
  useEffect(() => {
    ...
    setData(null)
    api
      .get(`/grade-tracker/classroom/${selectedId}`)
      .then((r) => setData(r.data))
      .finally(() => setLoading(false))
    // Fetch enrolled students for Custom Grades tab
    api.get(`/classrooms/${selectedId}`).then((r) => {
      setEnrolledStudents(
        r.data.enrollments.map((e: any) => ({ id: e.student.id, name: e.student.name }))
      )
    })
  }, [selectedId])
                      ...
                      data.students.map((s) => (
                        ...
                        >
                          <td className="px-5 py-3">
                            <button
                              onClick={() =>
                                navigate(`/teacher/grades/${s.studentId}?classroomId=${selectedId}`)
                              }
                          ...
                          <td className="px-5 py-3">
                            {s.category ? (
                              <span
                                className={`text-xs px-2 py-1 rounded-full border font-medium ${CATEGORY_STYLE[s.category]}`}
                              >
                                {s.category === 'needs_support'
                                  ? 'Needs Support'
                                  : s.category === 'meeting'
                                    ? 'Meeting'
                                    : 'Exceeding'}
