...
export default function StudentGradeDetail() {
  ...
  const {
    boundaries: storeBoundaries,
    loaded: storeBoundariesLoaded,
    load: loadBoundaries,
  } = useBoundaryStore()
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((r) =>
        setUnits(r.data.map((u: { id: string; name: string }) => ({ id: u.id, name: u.name })))
      )
  }, [classroomId])
  ...
  useEffect(() => {
    ...
    api
      .get(`/grade-tracker/student/${studentId}`, { params: { classroomId } })
      .then((r) => {
        setData(r.data)
        if (r.data.comment)
          setComment({
            strengths: r.data.comment.strengths ?? '',
            weaknesses: r.data.comment.weaknesses ?? '',
            notes: r.data.comment.notes ?? '',
          })
  ...
  function refresh() {
    if (!studentId || !classroomId) return
    api.get(`/grade-tracker/student/${studentId}`, { params: { classroomId } }).then((r) => {
      setData(r.data)
      setCustomItems(r.data.customBreakdown?.items ?? [])
    })
  }
      ...
      <nav className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            onClick={() =>
              navigate(
                classroomId ? `/teacher/grades?classroomId=${classroomId}` : '/teacher/grades'
              )
            }
        ...
        <div className="flex items-center gap-3">
          ...
          <TermIndicator />
          {data && (
            <button onClick={() => setShowPdfModal(true)} className="btn-3d-indigo px-3 py-1.5">
              Download Report
            </button>
          )}
        </div>
            ...
            <>
              <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
                ...
                {data.customBreakdown &&
                  (data.customBreakdown.totalCustomWeight > 0 ||
                    data.customBreakdown.quizWeight > 0) && (
                    <div className="mt-2">
                      ...
                      <GradeBreakdownBar
                        ...
                        customItems={data.customBreakdown.items.map((c) => ({
                          title: c.title,
                          weight: c.weight,
                          scorePct: c.scorePct,
                        }))}
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs text-gray-400">
                    Synced with Grade Tracker → Custom Grades
                  </span>
                </div>
                {customItems.length === 0 ? (
                  <p className="text-sm text-gray-400 text-center py-4 border border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                    No custom assignments yet. Add them in Grade Tracker → Custom Grades or
                  ...
                  <div className="space-y-2">
                    {customItems.map((a) => (
                      <div
                        key={a.id}
                        className="flex items-center gap-4 py-2 border-b border-gray-50 dark:border-gray-700/50 last:border-0"
                      >
          ...
          {activeTab === 'timeline' && (
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
              ...
              {timelineLoading ? (
                ...
                <p className="text-sm text-gray-400 text-center py-8 border border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                ...
                <ol className="relative border-l border-gray-200 dark:border-gray-700 space-y-4 pl-4">
                  {timeline.map((ev, i) => (
                    <li key={i} className="ml-2">
                      ...
                      {ev.detail && (
                        <p className="text-xs text-gray-500 dark:text-gray-400">{ev.detail}</p>
                      )}
                    </li>
                  ))}
                </ol>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
                ...
                >
                  <div className="flex gap-3 flex-wrap">
                    ...
                    >
                      {['Email', 'Phone', 'In person', 'Letter', 'Other'].map((t) => (
                  ...
                  <textarea
                    ...
                    onChange={(e) => setContactForm((f) => ({ ...f, summary: e.target.value }))}
                    required
                    rows={2}
                ...
                {contactsLoading ? (
                  ...
                  <div className="space-y-3">
                    {contacts.map((c) => (
                      <div
                        key={c.id}
                        className="flex items-start gap-3 py-3 border-b border-gray-50 dark:border-gray-700/50 last:border-0"
                      >
                        <span className="text-xs px-2 py-1 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 font-medium shrink-0">
                          {c.type}
                        </span>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-gray-800 dark:text-gray-200">{c.summary}</p>
                          {c.outcome && (
                            <p className="text-xs text-gray-500 mt-0.5">Outcome: {c.outcome}</p>
                          )}
                          <time className="text-xs text-gray-400">
