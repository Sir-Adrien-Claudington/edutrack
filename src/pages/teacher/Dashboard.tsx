...
export default function TeacherDashboard() {
  ...
  const [atRisk, setAtRisk] = useState<
    { classroomId: string; classroomName: string; students: AtRiskStudent[] }[]
  >([])
  ...
  useEffect(() => {
    api.get('/classrooms').then((r) => {
      ...
      Promise.all(
        rooms.map((c) =>
          api
            .get(`/students/classrooms/${c.id}/at-risk`)
            .then((res) => ({
              classroomId: c.id,
              classroomName: c.name,
              students: res.data as AtRiskStudent[],
            }))
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => setShowCreate(true)} className="btn-3d-indigo">
            + New Classroom
          </button>
        </div>
        ...
        {showCreate && (
          <form
            ...
            className="mb-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 space-y-3"
          >
            ...
            <div className="flex gap-2 justify-end">
              <button
                ...
                className="text-gray-500 dark:text-gray-400 px-3 py-2 text-sm"
              >
                Cancel
              </button>
              ...
              >
                ...
                <div className="flex gap-6 mt-4 items-baseline">
                  <div className="flex items-baseline gap-1.5">
                    <b className="text-lg font-semibold text-gray-900 dark:text-white tabular-nums leading-none">
            ...
            <div className="space-y-3">
              {atRisk.map((group) => (
                <div
                  ...
                  className="bg-white dark:bg-gray-800 border border-amber-200 dark:border-amber-800 rounded-xl p-4"
                >
                  ...
                  <div className="space-y-1">
                    {group.students.map((s) => (
                      <div key={s.studentId} className="flex items-center gap-3 text-sm">
                        ...
                        <Link
                          to={`/teacher/grades/${s.studentId}?classroomId=${group.classroomId}`}
                          className="ml-auto text-xs text-indigo-600 dark:text-indigo-400 hover:underline shrink-0"
                        >
                          View →
                        </Link>
                      </div>
