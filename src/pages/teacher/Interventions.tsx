...
export default function Interventions() {
  ...
  useEffect(() => {
    api.get('/students/interventions/all').then((r) => {
  ...
  async function moveStage(item: Intervention, stage: Stage) {
    const { data } = await api.put(
      `/students/${item.studentId}/interventions/${item.classroomId}`,
      { stage }
    )
  ...
  async function saveNotes() {
    if (!editing) return
    const { data } = await api.put(
      `/students/${editing.studentId}/interventions/${editing.classroomId}`,
      { notes: editNotes }
    )
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
                      ...
                      <div key={item.id} className={`rounded-lg border p-3 ${cfg.bg}`}>
                        ...
                        <div className="flex gap-1 mt-2 flex-wrap">
                          {STAGES.filter((s) => s !== stage).map((s) => (
                            <button
                              key={s}
                              onClick={() => moveStage(item, s)}
                              className="text-[10px] text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 px-1.5 py-0.5 rounded border border-gray-200 dark:border-gray-600 hover:border-gray-400 transition-colors"
                            >
                          ...
                          <button
                            ...
                            className="text-[10px] text-red-400 hover:text-red-600 px-1.5 py-0.5 rounded border border-red-100 hover:border-red-300 transition-colors ml-auto"
                          >
                            Remove
                          </button>
                        </div>
        ...
        {interventions.length === 0 && !loading && (
          <p className="text-center text-gray-400 text-sm mt-4">
            Students flagged as at-risk in your classrooms will appear here. Visit a classroom's
            At-Risk tab to add them.
          </p>
        )}
      </main>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-xl">
            <h2 className="font-semibold text-gray-900 dark:text-white mb-3">
              Notes — {editing.student.name}
            </h2>
            <textarea
              value={editNotes}
              onChange={(e) => setEditNotes(e.target.value)}
              rows={4}
              className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
              placeholder="Add intervention notes…"
            />
            <div className="flex gap-3 mt-4">
              <button onClick={saveNotes} className="btn-3d-indigo text-sm">
                Save
              </button>
              <button
