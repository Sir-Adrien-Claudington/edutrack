  ...
  async function save() {
    ...
    setPlans((ps) => {
      const filtered = ps.filter(
        (p) => !(p.dayOfWeek === editing.day && p.period === editing.period)
      )
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => shiftWeek(-1)}
              className="p-2 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400"
            >
              ‹
            </button>
            <span className="text-sm font-medium text-gray-700 dark:text-gray-300 w-52 text-center">
              {weekLabel}
            </span>
            <button
              onClick={() => shiftWeek(1)}
            ...
            <button
              ...
              className="text-xs text-indigo-600 hover:text-indigo-700 font-medium px-3 py-1.5 rounded-lg border border-indigo-200 hover:border-indigo-400 transition-colors"
            >
              This week
            </button>
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
          <div className="grid grid-cols-6 border-b border-gray-200 dark:border-gray-700">
            ...
            {DAYS.map((d) => (
              <div
                key={d}
                className="px-3 py-2 text-xs font-semibold text-gray-600 dark:text-gray-300 border-r border-gray-200 dark:border-gray-700 last:border-r-0"
              >
                {d}
              </div>
            ))}
            ...
            >
              ...
              {DAYS.map((_, di) => {
                ...
                return (
                  <div
                    key={di}
                    onClick={() => openCell(di, period)}
                    className="px-2 py-2 border-r border-gray-100 dark:border-gray-700 last:border-r-0 min-h-[56px] cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors"
                  >
                    {plan ? (
                      <div
                        className="rounded-md px-2 py-1.5 text-white text-xs leading-tight"
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-xl">
            <h2 className="font-semibold text-gray-900 dark:text-white mb-4">
              {DAYS[editing.day]} · Period {editing.period}
            </h2>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={save} className="btn-3d-indigo text-sm">
                Save
              </button>
              {editing.plan && (
                <button
