...
export default function Templates() {
  ...
  function addQuestion() {
    setQuestions((q) => [
      ...q,
      { text: '', type: 'SHORT_ANSWER', options: ['', '', '', ''], correctAnswer: '', points: 10 },
    ])
  }
  ...
  function openEdit(t: Template) {
    setForm({
      ...
      type: t.type,
      subject: t.subject ?? '',
      yearLevel: t.yearLevel?.toString() ?? '',
      instructions: '',
    })
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          {!isEditing && (
            <button onClick={() => setShowCreate(true)} className="btn-3d-indigo text-sm">
              + New Template
            </button>
          )}
        </div>
        ...
        {isEditing && (
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 mb-6">
            <h2 className="font-semibold text-gray-900 dark:text-white mb-4">
              {editModal ? 'Edit Template' : 'New Template'}
            </h2>
                  ...
                  >
                    <div className="flex gap-3 mb-2">
                      <input
                        ...
                        className="flex-1 border border-gray-300 dark:border-gray-600 rounded px-2 py-1.5 text-sm dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                      <select
                        value={q.type}
                        onChange={(e) => updateQuestion(i, 'type', e.target.value)}
                        className="border border-gray-300 dark:border-gray-600 rounded px-2 py-1.5 text-sm dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      >
                        {QTYPES.map((t) => (
                      ...
                      <input
                        ...
                        className="w-16 border border-gray-300 dark:border-gray-600 rounded px-2 py-1.5 text-sm dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                      <button
                        ...
                        {(q.options || ['', '', '', '']).map((opt: string, oi: number) => (
                          <input
                            ...
                            onChange={(e) => {
                              const o = [...(q.options || ['', '', '', ''])]
                              o[oi] = e.target.value
                              updateQuestion(i, 'options', o)
                            }}
                            placeholder={`Option ${oi + 1}`}
                            className="border border-gray-200 dark:border-gray-600 rounded px-2 py-1 text-xs dark:bg-gray-700 dark:text-white focus:outline-none"
                          />
                        ))}
                      </div>
                    )}
            ...
            <div className="flex gap-3">
              <button onClick={saveTemplate} className="btn-3d-indigo text-sm">
                Save Template
              </button>
              <button
                onClick={() => {
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {templates.map((t) => (
              <div
                key={t.id}
                className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-5 py-4 flex items-center justify-between"
              >
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-0.5">
                    {t.type}
                    {t.subject ? ` · ${t.subject}` : ''}
                    {t.yearLevel ? ` · Year ${t.yearLevel}` : ''} · {t.questions.length} questions
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-sm text-indigo-600 hover:text-indigo-700 font-medium px-3 py-1.5 rounded-lg border border-indigo-200 hover:border-indigo-400 transition-colors"
                  >
                    Use
                  </button>
                  <button
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-md shadow-xl">
            ...
            <div className="space-y-3">
              <div>
                ...
                <select
                  value={useForm.classroomId}
                  onChange={(e) => setUseForm((f) => ({ ...f, classroomId: e.target.value }))}
                  className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
                  {classrooms.map((c) => (
              ...
              <div>
                ...
                <input
                  type="date"
                  value={useForm.dueDate}
                  onChange={(e) => setUseForm((f) => ({ ...f, dueDate: e.target.value }))}
                  className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
            </div>
            <div className="flex gap-3 mt-5">
              <button onClick={useTemplate} className="btn-3d-indigo text-sm">
                Create Assignment
              </button>
              <button
