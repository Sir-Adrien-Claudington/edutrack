...
export default function Students() {
  ...
  useEffect(() => {
    api.get('/classrooms').then(async (r) => {
      ...
      const detailed = await Promise.all(
        cls.map((c: Classroom) => api.get(`/classrooms/${c.id}`).then((r) => r.data))
      )
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <TeacherNav activePage="students" />

      <main className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {all.length} student{all.length !== 1 ? 's' : ''} across {classrooms.length} classroom
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
            {filtered.map(({ student: s, classroom: c }) => (
              <div
                key={s.id}
                className="px-5 py-4 flex items-center gap-4 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors"
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    onClick={() =>
                      setQuickGrade({ studentId: s.id, studentName: s.name, classroomId: c.id })
                    }
