...
export default function UnitsSection({ classroomId, students, boundaries }: Props) {
  ...
  useEffect(() => {
    api
      .get(`/units/classroom/${classroomId}`)
      .then((r) => {
        setUnits(r.data)
  ...
  async function addLesson(unitId: string) {
    ...
    const { data } = await api.post(`/units/${unitId}/lessons`, {
  ...
  async function deleteLesson(unitId: string, lessonId: string) {
    ...
    setUnits((us) =>
      us.map((u) =>
        u.id !== unitId ? u : { ...u, lessons: u.lessons.filter((l) => l.id !== lessonId) }
      )
    )
  }

  function setUnderstandingOptimistic(
    unitId: string,
    lessonId: string,
    studentId: string,
    levelId: string | null
    ...
    api
      .put(
        `/units/lessons/${lessonId}/understanding/${studentId}`,
        { understandingLevelId: levelId },
        { signal: ctrl.signal }
      )
      .catch((err) => {
                      ...
                      : {
                          ...
                          understandings:
                            prev === null
                              ...
                              : [
                                  ...l.understandings.filter((u) => u.studentId !== studentId),
                                  { studentId, understandingLevelId: prev },
                                ],
                        }
                  ),
                }
  ...
  if (loading || !levelsLoaded)
    return <div className="text-sm text-gray-400 py-4">Loading units…</div>

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-gray-900 dark:text-white">Units</h2>
        <button onClick={() => setShowAddUnit(true)} className="btn-3d-indigo px-3 py-1.5">
          + Add Unit
        </button>
      </div>
      ...
      {showAddUnit && (
        <form
          onSubmit={addUnit}
          className="mb-4 flex gap-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4"
        >
          <input
            autoFocus
            value={newUnitName}
              ...
              <div className="flex items-center justify-between px-5 py-4">
                ...
                >
                  {editingUnitId === unit.id ? (
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') saveUnitName(unit.id, editingUnitName)
                      }}
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-xs text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 px-2 py-1"
                  >
                    Edit
                  </button>
                  <button
                    ...
                    <table className="min-w-full text-sm">
                      <thead>
                        <tr className="bg-gray-50 dark:bg-gray-700/50">
                          <th className="text-left px-4 py-2 text-xs font-medium text-gray-400 w-36 sticky left-0 bg-gray-50 dark:bg-gray-700/50">
                            Student
                          </th>
                          {unit.lessons.map((lesson) => (
                            <th key={lesson.id} className="px-2 py-2 text-center min-w-[90px]">
                              <div className="flex flex-col items-center gap-1">
                                ...
                                editingLesson.unitId === unit.id ? (
                                  <input
                                    autoFocus
                                    value={editingLessonTitle}
                                    onChange={(e) => setEditingLessonTitle(e.target.value)}
                                    onBlur={() =>
                                      saveLessonTitle(unit.id, lesson.id, editingLessonTitle)
                                    }
                                    onKeyDown={(e) => {
                                      if (e.key === 'Enter')
                                        saveLessonTitle(unit.id, lesson.id, editingLessonTitle)
                                  ...
                                  <button
                                    onClick={() => {
                                      setEditingLesson({ unitId: unit.id, lessonId: lesson.id })
                                ...
                                <input
                                  ...
                                  onBlur={(e) =>
                                    api.put(`/units/lessons/${lesson.id}`, {
                                ...
                                />
                                <button
                                  onClick={() => deleteLesson(unit.id, lesson.id)}
                          ...
                          <th className="px-2 py-2 text-center">
                            <button
                              ...
                              className="text-xs text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 font-medium border border-dashed border-indigo-300 rounded px-2 py-0.5"
                            >
                              + Lesson
                            </button>
                          </th>
                      ...
                      <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-50 dark:divide-gray-700/50">
                        {students.map((student) => {
                          ...
                          return (
                            ...
                            >
                              <td className="px-4 py-3 font-medium text-gray-900 dark:text-white sticky left-0 bg-white dark:bg-gray-800 text-sm">
                                {student.name}
                              </td>
                              ...
                              <td className="px-3 py-3 text-center">
                                {summative ? (
                                  <span
                                    ...
                                    style={{
                                      backgroundColor: summative.colour,
                                      color: contrastText(summative.colour),
                                    }}
                  ...
                  <div className="border-t border-gray-100 dark:border-gray-700 px-5 py-4 bg-gray-50/50 dark:bg-gray-700/20">
                    ...
                    <div className="flex flex-wrap gap-4">
                      {students.map((student) => {
                        ...
                        const pct =
                          score !== null ? Math.round((score / totalMarks) * 1000) / 10 : null
                        return (
                          <div
                            key={student.id}
                            className="flex flex-col items-start gap-1 min-w-[120px]"
                          >
                            ...
                            <div className="flex items-center gap-1">
                              <input
                                ...
                                onChange={(e) => {
                                  const v =
                                    e.target.value === ''
                                      ? null
                                      : Math.min(totalMarks, Math.max(0, Number(e.target.value)))
                              ...
                              {pct !== null && (
                                <GradeCell value={pct} boundaries={boundaries} readOnly size="sm" />
                              )}
                            </div>
                          </div>
                        )
